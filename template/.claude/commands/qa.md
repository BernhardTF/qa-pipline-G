...
Launch all 4 review agents **simultaneously** using the Agent tool. Pass each agent the full diff and the paths to relevant rubric files.

Use these `subagent_type` values:

- **`qa-security`** -- Pass the diff and the security checklist rubric path.
- **`qa-correctness`** -- Pass the diff and the review rules rubric path.
- **`qa-patterns`** -- Pass the diff, CLAUDE.md path, and pattern conventions rubric path.
- **`qa-docs`** -- Pass the diff and the CLAUDE.md path.

Each agent prompt must include the full diff content and instruct the agent to read the rubric files. The agents already know their review procedures.
