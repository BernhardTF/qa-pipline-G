---
name: qa-fixit
description: Fix issues found by the /qa review pipeline. Reads the latest review report, applies fixes in batch, updates the report with fix status and new line references. Use when the user says "fix the qa issues", "fixit", or types /qa-fixit.
---
...
Create a task for each finding to track progress. Order by severity (BLOCK first, then WARN) and by file (group fixes in the same file together to minimize line-shift conflicts).
